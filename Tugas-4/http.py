...
import base64
import urllib.parse  # ← tambahan untuk parsing query
...
class HttpServer:
	...
	# method tambahan: menampilkan daftar file
	def list(self):
		files = os.listdir('.')
		file_list = '\n'.join(files)
		return self.response(200, 'OK', file_list, {'Content-type': 'text/plain'})

	# method tambahan: mengupload file
	def upload(self, headers, body):
		...
		if filename:
			try:
				# body di-encode base64 dari client, jadi decode dulu
				decoded_body = base64.b64decode(body.encode())
				with open(os.path.join('.', filename), 'wb') as f:
					f.write(decoded_body)
				print(f"[INFO] File {filename} berhasil disimpan ({len(decoded_body)} bytes)")
				return self.response(200, 'OK', f'File {filename} berhasil diupload', {'Content-type': 'text/plain'})
			except Exception as e:
				return self.response(500, 'Internal Server Error', f'Gagal upload: {str(e)}'.encode())
		return self.response(400, 'Bad Request', 'Filename header missing', {})

	# method tambahan: menghapus file
